...
class ModelMaker(object):
  ...
  def fit_data(self):
    ...
    self.model.save_weights('bestLocationsWeights.h5',overwrite=True)
...
def generate_hyperparams(n_trials):
  ...
  n_trials = 1
  learning_rates = [8.2e-4] #[0.001, 0.0009, 0.0007, 0.0005]
  reg_strengths = [8.5e-5]#10**np.random.uniform(-5,-4,n_trials).astype('float32')
  dropout_probs = [0.44]#[0.4, 0.4, 0.4, 0.4]
...
def build_ensembles(hyperparams_list):
  ...
  for trial in range(n_trials):
    ...
    if augment:
      ...
      np.save('locations_train_predictions.npy',train_predictions)
      np.save('locations_test_predictions.npy', test_predictions)
      np.save('locations_train_answers.npy',y_train)
      np.save('locations_test_answers.npy', y_test)
    # solver.plot(trial)
    ensemble_results.append(test_predictions)

  solver.plot('locationsMar8')
