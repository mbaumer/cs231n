...
if env == 'local':
  ...
  training_input = '/data/X_genres96.npy'
  training_output = '/data/Y_genres96.npy'
...
class ModelMaker(object):
  ...
  def fit_data(self):
    ...
    self.model.save_weights('bestGenresWeights.h5',overwrite=True)
...
class CrossValidator(object):
  ...
  def plot(self, trial):
    ...
    history_dict =  [self.batch_histories, self.epoch_histories]
    pickle.dump(history_dict,open('genres_history_dict.out','wb'))
...
def generate_hyperparams(n_trials):
  ...
  n_trials = 1
  learning_rates = [1.2e-5] #[0.001, 0.0009, 0.0007, 0.0005]
  reg_strengths = [1e-3]#10**np.random.uniform(-5,-4,n_trials).astype('float32')
  dropout_probs = [0.35]#[0.4, 0.4, 0.4, 0.4]
...
def build_ensembles(hyperparams_list):
  ...
  for trial in range(n_trials):
    ...
    if augment:
      ...
      np.save('genres_train_predictions.npy',train_predictions)
      np.save('genres_test_predictions.npy', test_predictions)
      np.save('genres_train_answers.npy',y_train)
      np.save('genres_test_answers.npy', y_test)
    # solver.plot(trial)
    ensemble_results.append(test_predictions)

  solver.plot('genresMar8')
